@if( $chat )
    @if ( $receiverUser->id == $chat->receiver_id )
        <div class="pt-4 flex flex-col justify-end text-right">
            {{-- <span class="font-medium text-sm font-semibold text-right">{{ auth()->user()->name }}</span> --}}
...
@else
    <div class="pt-4 flex flex-col justify-start">
        <div class="rounded-lg bg-white text-slate-900 shadow-xl shadow-black/5 ring-1 p-3 flex flex-col text-center">
            <span class="text-normal">Send your first <i class="text-indigo-900 font-bold">Hey!</i> message to Welcome.</span>
        </div>
    </div>
@endif
